  ...
  _poiInfo: function(tags,id) {
    ...
    if(tags['name']){
      ...
      has_title=true;
        ...
        if(tags['shop']){titel=tags['shop'];}
        if(tags['amenity']){titel=tags['amenity'];}
        if(tags['tourism']){titel=tags['tourism'];}
    }
    r.append($('<h3>').text(titel));
   ...
   if(has_title){
    ...
    if((titel!='yes')&&(has_title))r.append($('<p>').text(titel));

    //address
    if(tags['addr:street']){
       r.append($('<span>').text(tags['addr:street']));
       if(tags['addr:housenumber']){
          r.append($('<span>').text(' '+tags['addr:housenumber']));
       }
       r.append('<br>');
    }
    if(tags['addr:city']){
       if(tags['addr:postcode']){
          r.append($('<span>').text(tags['addr:postcode']+' '));
       }
       r.append($('<span>').text(tags['addr:city']));
       r.append('<br><br>');
    }
    ...
    if(tags['opening_hours']){
      r.append('<br>');
      r.append($('<span>').text(tags['opening_hours']));
      r.append('<br><br>');
    }
    //note & decription
    if(tags['description']){
      r.append($('<b>').text(tags['description']));
      r.append('<br>');
    }

    if(tags['note']){
      r.append($('<p>').text(tags['note']));
      r.append('<br><br>');
    }
    ...
    //telefon
    if(tags['phone']){
      r.append($('<span>').text(tags['phone']));
      r.append('<br>');
    }
    if(tags['contact:phone']){
      r.append($('<span>').text(tags['contact:phone']));
      r.append('<br>');
    }

    //email
    if(tags['email']){
      r.append($('<span>').text(tags['email']));
      r.append('<br>');
    }
    if(tags['contact:email']){
      r.append($('<span>').text(tags['contact:email']));
      r.append('<br>');
    }

    //website keys: [contact:website|website]
    if(tags['website']){
      r.append($('<span>').text(tags['website']));
      r.append('<br>');
    }
    if(tags['contact:website']){
      r.append($('<span>').text(tags['contact:website']));
      r.append('<br>');
    }


    for (key in tags){
 //     if((key!='name')&&(key!='addr:housenumber')&&(key!='addr:street')&&(key!='addr:city')&&(key!='addr:postcode')){
     if(false){   
